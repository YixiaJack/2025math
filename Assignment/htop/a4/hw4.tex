...
\begin{align*}
   ...
   \begin{proof}
    ...
    With this definition, $Y = F^{-1}(X)$ will have distribution function $F$ even when $F$ is not continuous or strictly increasing, provided that $F$ is at least right-continuous and non-decreasing (as any proper distribution function must be).
   \end{proof}
...
\textbf{Case 1: Symmetric random walk ($p = q = 1/2$)}
...
\textbf{Case 2: Asymmetric random walk ($p \neq q$)}
