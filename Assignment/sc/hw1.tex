...
plt.semilogy(list(ns_d), err_fib_d, 'b.-', ms=3, label='Double')
plt.semilogy(list(ns_s), err_fib_s, 'r.-', ms=3, label='Single')
...
\begin{itemize}[nosep]
  \item Single precision has $2^{24} \approx 1.7 \times 10^7$, so error is exactly zero for $n \lesssim 35$ and reaches complete loss (error $\geq 1$) by $n \approx 40$.
  \item Double precision has $2^{53} \approx 9 \times 10^{15}$, so error is exactly zero for $n \lesssim 75$ and reaches complete loss by $n \approx 80$.
\end{itemize}

The behavior is qualitatively the same: a sharp transition from zero error to total loss within just a few steps, due to the exponential amplification factor $\varphi^{2n}$. The transition simply shifts later in double precision because exact integer arithmetic extends to a much wider range.
\medskip
...
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
axes[0].semilogy(list(ns_d), err_fib_d, 'b.-', ms=3, label='Double')
axes[0].semilogy(list(ns_s), err_fib_s, 'r.-', ms=3, label='Single')
...
for n in [10, 20, 30, 50, 100, 130, 150, 200, 500, 1000]:
    ...
    print(f"n={n:6d}  max(double)={np.max(ad):15.6e}  "
          f"a_nn(double)={ad[n]:18.10e}  a_nn(single)={as_[n]:18.10e}")
\end{lstlisting}
...
\hline
10 & $2.52 \times 10^{2}$ & 1.0000000000 & $2.52 \times 10^{2}$ & 1.0000000000 \\
20 & $1.85 \times 10^{5}$ & 1.0000000000 & $1.85 \times 10^{5}$ & 1.0000000000 \\
30 & $1.55 \times 10^{8}$ & 1.0000000000 & $1.55 \times 10^{8}$ & 1.0000000000 \\
50 & $1.26 \times 10^{14}$ & 1.0000000000 & $1.26 \times 10^{14}$ & 0.9999999404 \\
100 & $1.01 \times 10^{29}$ & 1.0000000000 & $1.01 \times 10^{29}$ & 0.9999992251 \\
130 & $9.51 \times 10^{37}$ & 1.0000000000 & \texttt{inf} & \texttt{inf} \\
150 & $9.28 \times 10^{43}$ & 1.0000000000 & \texttt{inf} & \texttt{inf} \\
200 & $9.05 \times 10^{58}$ & 1.0000000000 & \texttt{inf} & \texttt{inf} \\
500 & $1.17 \times 10^{149}$ & 1.0000000000 & \texttt{inf} & \texttt{inf} \\
...
for n in [10, 20, 50, 100, 200, 500, 1000, 1020, 1025, 1030]:
    ...
    print(f"n={n:6d}  E(k)={Ek:20.10e}  n/2={exact:10.1f}  "
          f"|error|={err:15.6e}  Mn={Mn:20.6e}")
\end{lstlisting}

\begin{center}
\begin{tabular}{r|cccc}
$n$ & $E(k)$ & $n/2$ & $|\text{error}|$ & $M_n$ \\
\hline
10 & 5.000000e+00 & 5.0 & 0.000000e+00 & $2.52 \times 10^{2}$ \\
20 & 1.000000e+01 & 10.0 & 0.000000e+00 & $1.85 \times 10^{5}$ \\
50 & 2.500000e+01 & 25.0 & 0.000000e+00 & $1.26 \times 10^{14}$ \\
100 & 5.000000e+01 & 50.0 & 7.105427e-15 & $1.01 \times 10^{29}$ \\
200 & 1.000000e+02 & 100.0 & 0.000000e+00 & $9.05 \times 10^{58}$ \\
500 & 2.500000e+02 & 250.0 & 1.421085e-13 & $1.17 \times 10^{149}$ \\
1000 & 5.000000e+02 & 500.0 & 3.410605e-13 & $2.70 \times 10^{299}$ \\
1020 & \texttt{inf} & 510.0 & \texttt{inf} & $2.81 \times 10^{305}$ \\
1025 & \texttt{NaN} & 512.5 & \texttt{inf} & \texttt{inf} \\
1030 & \texttt{NaN} & 515.0 & \texttt{inf} & \texttt{inf}
\end{tabular}
\end{center}
...
For $n = 1020$, $M_n$ is still finite but the sum $\sum k \cdot a_{n,k}$ overflows, making $E(k) = \texttt{inf}$. For $n = 1025$, the peak $a_{n,n/2}$ itself exceeds $10^{308}$ and overflows to \texttt{inf}, making $M_n = \texttt{inf}$. The sum then involves $\texttt{inf} - \texttt{inf}$ or $\texttt{inf}/\texttt{inf}$ operations, producing $E(k) = \texttt{NaN}$.
...
\begin{itemize}[nosep]
  \item $n = 10$: $k \in [0, 10]$, i.e.\ all values
  \item $n = 20$: $k \in [1, 19]$
  \item $n = 50$: $k \in [11, 39]$
\end{itemize}
...
for n, style in [(10, 'b-'), (20, 'r--'), (50, 'g-.')]:
    ...
    mu = n / 2.0
    sigma = np.sqrt(n / 4.0)
    # Left: chosen k range, shifted to center
...
\caption{Left: $a_{n,k}/M_n$ vs.\ $k - n/2$ over chosen $k$ ranges. Right: Standardized curves collapse onto Gaussian $e^{-z^2/2}$.}
\end{figure}
\end{document}