...
\begin{solution}
    \begin{itemize}
        \item  Only at points \(a = x_i\) or \(a = x_i-1\) (for \(x_i\in P\)) does the count in \((a,a+1]\) change. Thus, define
        \[
        Q = \{x_i-1 : x_i \in P\} \cup \{x_i : x_i \in P\}.
        \]
        \item \textbf{Greedy:} Initialize \(L=\min(P)-1\) (the minimum allowed left endpoint) and scan \(Q\) for the smallest candidate \(a \ge L\) such that the count of points in \((a, a+1]\) is at least \(k\). Once such an \(a\) is found, choose that interval, update \(L\) to \(a+1\), and repeat.
        \item \textbf{Correctness:} By always choosing the leftmost feasible candidate:
        \begin{itemize}
    ...
    \begin{lstlisting}
    HEAVY-INT(P, k)
        Input: sorted P = [x1, x2, ..., xn], positive integer k
        Output: set S of disjoint heavy unit intervals (a, a+1]
    
        S <- {}             // Solution set
        L <- min(P) - 1     // Allowed left endpoint
        Q <- merge({x - 1 for x in P}, {x for x in P})
                            // Q is sorted and has at most 2n candidates
    
        p_left <- 1, p_right <- 1
    
        // Find first candidate a_star >= L with count >= k
